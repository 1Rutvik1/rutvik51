...
/// </summary>
public class GameController :MonoBehaviourPunCallbacks, IOnEventCallback, IPunInstantiateMagicCallback
{
    ...
    readonly List<CarController> m_AllCars = new List<CarController>();
	...
	void Awake ()
	{
		if (!WorldLoading.HasLoadingParams)
		{
			...
			LoadingScreenUI.LoadScene (RegimeForDebug.RegimeSceneName, LoadSceneMode.Additive);
		}
		Instance = this;
		...
		if (WorldLoading.IsMultiplayer)
		{
			...
			else if (((GameMode)PhotonNetwork.LocalPlayer.CustomProperties[C.GameType]) == GameMode.Spectators)
			{
				InitRaceEntity();

				StartCoroutine(ActiveSpectatorsCars());
			}
		...
		m_PlayerCar = Instantiate(WorldLoading.PlayerCar.CarPrefab);
		...
		for (int i = 0; i < WorldLoading.AIsCount; i++)
		{
			...
			var car = Instantiate(carPreset.CarPrefab);
	...
	IEnumerator ActiveSpectatorsCars()
    {
		yield return new WaitForSeconds(1);

		PlayerManager.Manager.instantiatedCars.ForEach(c => c.gameObject.GetComponent<MultiplayerCarController>().SpectatorEnable());
	}
	...
	IEnumerator StartRaceCoroutine ()
	{
		...
		if (WorldLoading.IsMultiplayer)
		{
			Invoke(nameof(SetCameraForSpectators), 0.5f);

			if (PlayerManager.Manager.gameMode == GameMode.Clients && !(bool)PhotonNetwork.CurrentRoom.CustomProperties[C.RaceStarted])
			{
				...
				{
					Hashtable hashtable = new Hashtable() { { C.RaceStarted, true } };
	...
	void SetCameraForSpectators()
    {
		if (((GameMode)PhotonNetwork.LocalPlayer.CustomProperties[C.GameType]) == GameMode.Spectators)
		{
			...
			{
				Debug.Log("SetCamera");
				CameraController.Instance.OverrideCar = AllCars[0];
			}
		}
	}
    ...
    readonly List<MultiplayerCarController> MultiplayerCars = new List<MultiplayerCarController>();
	...
	IEnumerator InstantiateMultiplayerCar ()
	{
		...
		foreach (var player in orderedPlayers)
		{
			...
			if (((GameMode)player.Value.CustomProperties[C.GameType]) == GameMode.Clients)
				index++;
		}

		Debug.Log(PhotonNetwork.LocalPlayer.CustomProperties.ContainsKey(C.GameType));
